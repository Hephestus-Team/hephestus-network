...
exports.patch = async (req, res, next) => {
	...
	let nullDictionary = ["birthday", "first_name", "last_name", "visibility"];
	let cannotBeNull = [];
	...
	properties.forEach((property) => {
		...
		if(nullDictionary.includes(property)) cannotBeNull.push(property);
	});

	// CHECK IF NULL DICTIONARY PROPERTIES ARENT NULL
	let isNull = cannotBeNull.some(property => property === null);

	if (isNull) return res.status(403).send({ message: { syntax: "Cannot send empty value for first name, last name, birthday nor visibility" } });
